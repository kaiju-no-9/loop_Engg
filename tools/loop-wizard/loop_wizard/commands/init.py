...
def build_workflow_yml(pattern: str, cadence: str, agent_tool: str) -> str:
    ...
    meta = AGENT_METADATA.get(agent_tool, {})
    run_cmd_template = meta.get("run_cmd", "{agent_tool} /loop {pattern}")
    run_cmd = run_cmd_template.format(pattern=pattern, agent_tool=agent_tool)
...
def run_init_flow(target_dir, pattern, agent_tool, non_interactive, overrides, reconfigure):
    ...
    if not agent_tool:
        if non_interactive:
            ...
            console.print()
            console.print(Panel(
                "[bold white]Which AI coding agent will run this loop?[/bold white]\n"
                "[dim]Choose based on the tools you already have installed.[/dim]",
                title="[bold magenta]⚡ SELECT AGENT TOOL ⚡[/bold magenta]",
                title_align="left",
                border_style="magenta",
                padding=(1, 2),
                expand=False,
            ...
            tool_table.add_column("#", style="bold cyan", width=4, justify="right")
            tool_table.add_column("Tool", style="bold white", min_width=16)
            tool_table.add_column("Provider", style="dim cyan", min_width=22)
            tool_table.add_column("Description", style="dim")

            for i, tool in enumerate(AGENT_TOOLS, 1):
                meta = AGENT_METADATA.get(tool, {})
                icon = meta.get("icon", "●")
                provider = meta.get("provider", "")
                desc = meta.get("description", "")
                style = "" if i != 1 else "bold"
                rec = "  [italic green](recommended)[/italic green]" if i == 1 else ""
                tool_table.add_row(
                    f"{i}.",
                    f"{icon}  {tool}{rec}",
            ...
            choice = Prompt.ask(
                "\n  [cyan]▸ Select option (1-" + str(len(AGENT_TOOLS)) + ")[/cyan]",
    ...
    # ── Done ────────────────────────────────────────────────────────
    # Build per-tool run command for completion output
    meta = AGENT_METADATA.get(agent_tool, {})
    run_cmd_template = meta.get("run_cmd", "{agent_tool} /loop {pattern}")
    tool_run_cmd = run_cmd_template.format(pattern=pattern, agent_tool=agent_tool)
    tool_dry_run_cmd = tool_run_cmd + " --dry-run"
    ...
    completion_lines.append(f"     $ [bold]{tool_dry_run_cmd}[/bold]")
    ...
    completion_lines.append(f"     $ [bold]{tool_run_cmd}[/bold]")
